...
import { BrainCircuit, Copy, CheckCircle2 } from "lucide-react";
  ...
  return (
    <div className="flex flex-col bg-[#0d0d14] rounded-xl border border-indigo-900/60 shadow-[0_0_40px_-15px_rgba(79,70,229,0.25)] overflow-hidden mt-6 animate-in slide-in-from-bottom-8 fade-in duration-700">
      ...
      <div className="p-6 space-y-6">
        <div className="bg-[#0a1a0a] rounded-xl p-5 border border-green-900/30 shadow-inner overflow-x-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {report.failed_rules_summary.map((rule, idx) => (
        ...
        <Button 
          variant="outline" 
          onClick={handleCopy}
          className="w-full h-11 mt-4 border-indigo-900/50 text-indigo-300 bg-indigo-950/10 hover:bg-indigo-950/50 hover:text-indigo-200 transition-colors font-semibold"
        >
          {copied ? (
            <><CheckCircle2 className="w-4 h-4 mr-2 text-green-500" /> Copied to Clipboard</>
          ) : (
            <><Copy className="w-4 h-4 mr-2 opacity-70" /> Copy Report</>
          )}
        </Button>
      </div>
    </div>
